...
bool ends_with_dot(char *string);
...
int main (int argc, char *argv[])
{
  ...
  markov_chain = new_markov_chain ((print_func_t) printf,
                                   (comp_func_t) strcmp,
                                   (copy_func_t) strdup,
                                   (free_data_t) free,
...
char *strdup(const char *str)
{
    char *duplicated_str = (char *)malloc((sizeof *duplicated_str) * strlen
            (str));
...
void usage (char *program_name)
{
  fprintf (stdout, USAGE_FMT, basename (program_name));
}
...
*num_of_tweets, int *num_of_words, FILE **text_corpus_fp)
{
  ...
  // valid
  *seed = (unsigned int) strtol (argv[SEED_ARG_INDEX], &end_ptr, DECIMAL_BASE);
  *num_of_tweets =
  ...
  *num_of_words = READ_ALL_WORDS;
  ...
  {
    *num_of_words =
        (int) strtol (argv[WORD_COUNT_ARG_INDEX], &end_ptr, DECIMAL_BASE);
  }
